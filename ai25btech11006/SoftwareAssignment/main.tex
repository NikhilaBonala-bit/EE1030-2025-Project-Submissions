...
A = U \Sigma V^T
\]
where \(U\) and \(V\) are orthogonal matrices, that is their columns are orthonormal vectors and \(\Sigma\) contains the singular values.
...
This decomposition reveals the geometry of linear transformations. Large singular values are responsible for maximum stretch. In image compression, using the top \(k\) singular values preserves the most important information while discarding small details.
...
\begin{itemize}
    ...
    \item Larger \(k\): less compression, more quality
